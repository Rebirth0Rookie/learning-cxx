...
unsigned long long fibonacci(int i) {
    switch (i) {
        ...
        default:
            // TODO: 补全三目表达式缺失的部分
            return arr[i] ? arr[i] : (arr[i] = fibonacci(i - 1) + fibonacci(i - 2));//arr[i]就表示fib(i)，计算过就返回，没计算过就算
    }
}

int main(int argc, char **argv) {
    // TODO: 为此 ASSERT 填写正确的值
    ASSERT(sizeof(arr) == 720, "sizeof array is size of all its elements"); // longlong占8字节64比特位
    // ---- 不要修改以下代码 ----
